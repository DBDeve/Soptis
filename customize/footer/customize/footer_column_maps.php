<?php
    ...
    for ( $i = 1; $i <= 4; $i++ ) {
        ...
        $wp_customize->add_setting( "footer_column_{$i}_maps", array(
            'default'           => false,
            'sanitize_callback' => 'rest_sanitize_boolean',
            'transport'         => 'refresh',
        ));
        $wp_customize->add_control( "footer_column_{$i}_maps_control", array(
            'label'    => "column {$i} maps",
            'section'  => "tema_footer_column_{$i}",
            'settings' => "footer_column_{$i}_maps",
            'type'     => 'checkbox',
        ) );
        ...
        $wp_customize->add_setting( "footer_column_{$i}_maps_align", array(
            'default'           => 'center',
        ...
        $wp_customize->add_control( "footer_column_{$i}_maps_align_control", array(
            'label'    => " column {$i} maps alignment ",
            'section'  => "tema_footer_column_{$i}",
            'settings' => "footer_column_{$i}_maps_align",
            'type'     => 'select',
            'choices'  => $allowed_justify_content_footer_maps,
            'active_callback' => "show_maps_{$i}",
        ) );

        $wp_customize->add_setting( "footer_column_{$i}_maps_link", array(
        ...
        $wp_customize->add_control( "footer_column_{$i}_maps_link_control", array(
            'label'    => "column {$i} maps link",
            'section'  => "tema_footer_column_{$i}",
            'settings' => "footer_column_{$i}_maps_link",
            'type'     => 'textarea',
            'active_callback' => "show_maps_{$i}",
        ) );
    ...
    function show_maps_1() { 
        return get_theme_mod( "footer_column_1_maps", false ) === true;
    };
    function show_maps_2() { 
        return get_theme_mod( "footer_column_2_maps", false ) === true;
    };
    function show_maps_3() { 
        return get_theme_mod( "footer_column_3_maps", false ) === true;
    };
    function show_maps_4() { 
        return get_theme_mod( "footer_column_4_maps", false ) === true;
    };
